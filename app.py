...
def transcribe():
    ...
    try:
        ...
        # LOGICA DI GENERAZIONE RIASSUNTO E MAPPA (FALLBACK)
        summary = ""
        concepts = []
        
        if text:
            # Creiamo il riassunto prendendo le prime frasi significative
            sentences = [s.strip() for s in text.split('.') if len(s.strip()) > 5]
            if len(sentences) > 0:
                summary = ". ".join(sentences[:3])
                if len(sentences) > 3: summary += "..."
            
            # Creiamo la mappa concettuale prendendo frasi medie
            concepts = [s.strip() for s in text.split('.') if 10 < len(s.strip()) < 80][:5]
